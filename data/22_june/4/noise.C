void noise() {
  ...
  for (Int_t i=0; i<SingleScatterCutTree->GetEntries(); i++) {
    ...
    CoinEnergyDepWithNoise = randgen->Gaus(CoinEnergyDep*1000, 0.112)/1000;
  ...
  TH1F *h3 = new TH1F("h3", "Single Scatter cut (0-400KeV) with noise", 80, 0, 400);
  ...
  TH1F *h4 = new TH1F("h4", "Single Scatter cut (50-300KeV) with noise", 50, 50, 300);
  h4->SetTitle("Single-scatter cut (50-300KeV) with noise");
  h4->GetXaxis()->SetTitle("Energy [KeV]");
  h4->GetYaxis()->SetTitle("Counts");
  TCanvas* c4 = new TCanvas("c4", "Single Scatter cut (50-300KeV) with noise", 1024, 768);
  c4->cd();
  SSCT_50_300kev->Draw("CoinEnergyDep*1000>>h4");
  c4->SetLogy();
  c4->SaveAs("ssc_50_300_with_noise.png");
}
